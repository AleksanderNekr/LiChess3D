...
export function ChessSetup(props: { setOrbitEnabled: (enabled: boolean) => void }) {
	...
	const getPiecePositions = useCallback(() => chess.board()
			.flatMap((row, rowIndex) =>
				row.map((piece, colIndex) => {
					...
					return {
						...
						square,
						capturable: validMoves.includes(square) && piece.color !== turn, // Mark as capturable if it's
					                                                                     // in validMoves and not the
					                                                                     // same color as the current
					                                                                     // turn
						inCheck: piece.type === "k" && chess.inCheck() && piece.color === turn, // Highlight the king
					                                                                            // if it's in check
					};
				}),
			)
			.filter(Boolean),
		[ chess, turn, validMoves ]);
	...
	const updateGameStateCallback = useCallback((chessPosition: Chess, lastMove: string, lastMoveAbsNumber: number) => {
	...
	}, [ gameId, streamGameState, updateGameStateCallback ]);

	useEffect(() => {
		setPiecePositions(getPiecePositions());
	}, [ getPiecePositions, stateUpdated ]);
